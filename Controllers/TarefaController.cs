...
namespace TrilhaApiDesafio.Controllers
{
    ...
    public class TarefaController : ControllerBase
    {
        ...
        public IActionResult ObterPorTitulo(string titulo)
        {
            ...
            if (!tarefas.Any())
                return NotFound(new { Message = $"Nenhuma tarefa encontrada com o título '{titulo}'." });
        ...
        {
            // Buscar as tarefas no banco utilizando o EF, que contenham a data recebida por parâmetro
            var tarefas = _context.Tarefas.Where(x => x.Data.Date == data.Date).ToList();

            if (!tarefas.Any())
                return NotFound(new { Message = $"Nenhuma tarefa encontrada na data {data.Date.ToShortDateString()}." });
        ...
        public IActionResult ObterPorStatus(EnumStatusTarefa status)
        {
            ...
            if (!tarefas.Any())
                return NotFound(new { Message = $"Nenhuma tarefa encontrada com o status '{status}'." });
